...
#else
#define ptcbuffsize 5
static int Pulse_to_calcu[ptcbuffsize] = {10,200,400,550,700};
#define stoppulse 800
#endif
				...
				if(!TIMERCOUNTER++){ //first time
				  ...
				  memcpy(&DISTANCE_DATA[1],Cover_data,sizeof(Cover_data));
					...
					for(int i=0;i<ptcbuffsize;i++){
						...
						// transmit raw data.....   1 ms laser after 					
						if(TIMERCOUNTER == Pulse_to_calcu[i]+10){ // 1 ms later, all data should be OK now..
							if(i == ptcbuffsize -1)
									TXRAWBUFF[1] = i + 100; // the i th data, if it is the last one, plus 100
							else
								  TXRAWBUFF[1] = i; // the i th data
							TXRAWBUFF[2] = Pulse_to_calcu[i]; // the i th data's pulses... 
							HAL_UART_Transmit_DMA(&huart2,(uint8_t *)TXRAWBUFF,sizeof(TXRAWBUFF));
